...
**Step 2.5 — If still stuck, schedule a re-check before you end the turn.** "Tell the partner I'm on it and wait" is not a recovery strategy — queue a re-check (`claude-assistant remind add "re-check the blocked step" --contact "<this chat_id>" --in 2m`) so the blocker self-resolves if the dependency comes back, and mark the open commitment with `claude-assistant commitment set "<what>"` (clear it with `claude-assistant commitment clear` when resolved). The daemon's blocked-session watchdog will nudge an idle session that still has a commitment — never re-park after a nudge.

**Step 3 — If you still need human help, route to admin (the owner) with a specific ask, not the partner.** The partner shouldn't have to debug infrastructure. If something blocks their request, tell them warmly that you're working on it and ping admin in parallel with a concrete ask like "found runaway native_host PID 11522 at 100% CPU; killed it; need you to click the chrome-control extension icon." Keep the re-check alive after escalating. Full discipline: see "Self-Heal Before Escalating" + "Never Idle-Abandon a Task" in `~/.claude/CLAUDE.md`.
